 ...
 */
public class GravatarSourceService extends SourceServiceFacadeImpl implements
	...
	/**
	 * List with Email that dont have an image, will be used when default image
	 * is set to 404
	 */
	private List<Email> emailsWith404;
	...
	public boolean initialize(Source configuration) {
		...
		if (isInitialized()) {
			// localy keep configured properties
			imageSize = getImageSize();
			defaultImage = getDefaultImage();
			rating = getRating();

			if (defaultImage.equals("404")) {
				// prepare list for emails to recheck
				emailsWith404 = new LinkedList<Email>();
			}
		}
	...
	protected void updateDataSet() {
		...
		if(emailsWith404 == null || emailsWith404.isEmpty()) {
			// nothing to do
		}
		
		// copy list
		LinkedList<Email> tmpList = new LinkedList<Email>(emailsWith404);
		// clear original list
		emailsWith404.clear();
		// and recheck all which may lead to re adding to 404 list
		for(Email email : tmpList) {
			enrichInformationObjectsOfEmail(email);
		}
		
	}
	/**
	 * Attaches a gravatar image to all information objects the given email
	 * belongs to ({@link Email#getInformationObjects()})
	 * 
	 * @param email
	 *            Email Address to be used for retrieval of gravatar image
	 */
	public void enrichInformationObjectsOfEmail(Email email) {
		...
		EList<InformationObject> informationObjects = email
		...
		if (imageUrl == null) {
			log("Could not create gravatar image url for " + email.getAdress(),
		...
		if (defaultImage.equals("404")) {
			URL requestUrl;
			// check availability to prevent from adding invalid images
			try {
				requestUrl = new URL(imageUrl);
			} catch (MalformedURLException e) {
				log("No valid gravatar image url: " + imageUrl, LogService.LOG_WARNING);
				return;
			}
			HttpURLConnection connection;
			try {
				connection = (HttpURLConnection) requestUrl.openConnection();
				// read only head for performance
				connection.setRequestMethod("HEAD");
				connection.connect();
				if(connection.getResponseCode() == 404) {
					log("No gravatar image for " + email.getAdress(), LogService.LOG_DEBUG);
					// add email to lookup late
					emailsWith404.add(email);
					// and skip for now
					return;
				}
			} catch (IOException e) {
				log("Could not check if gravatar image exists for " + imageUrl + " due to " + e.getMessage(), LogService.LOG_WARNING);
				return;
			}
		}
		...
		for (InformationObject informationObject : informationObjects) {
			Image attachedImage = enrichInfomationObject(informationObject,
					imageUrl);
	 ...
	 *            Information object to enrich
	 * @param email
	 *            Email address used for retrieval of gravatar image
	 */
	public void enrichInformationObject(InformationObject informationObject,
			Email email) {
		...
		if (!email.getInformationObjects().contains(informationObject)) {
			// not allowed to enrich information object with value of not
			// connected website
			return;
		}
	...
	/**
	 * Attaches an image with the given image url to the given information
	 * object.
	 * 
	 * @param informationObject
	 *            Information object to attach gravatar image to
	 * @param imageUrl
	 *            Url of the gravatar image
	 * @return The attached image
	 */
	private Image enrichInfomationObject(InformationObject informationObject,
			String imageUrl) {

		String ioImageIdent = informationObject.getIdent() + "_"
				+ imageUrl.hashCode();
		...
		log("Added gravatar image " + imageUrl + " to "
				+ informationObject.getName(), LogService.LOG_INFO);

		return gravatarImage;
	}

	/**
	 * Creates an url of a gravatar image using a hash of the email address. The
	 * size will be determined in the source configuration.
	 * 
	 * @param emailAddress
	 *            Address that should be used for the image retrieval
	...
	private String createImageUrl(String emailAddress) {
		...
		return "http://www.gravatar.com/avatar/" + emailHash + "?s="
				+ imageSize + "&r=" + rating + "&d=" + defaultImage;
	}
	 ...
	 * 
	 * @param emailAddress
	 *            Address to be used for parameter generation.
	 * 
	...
	/**
	 * Determine the gravatar rating parameter out of the configuration entry or
	 * the default value.
	 * 
	 * @return The set rating, or the default if not set.
	 */
	private String getRating() {
		return source.getPropertyValueElseDefault(
				GravatarProperties.RATING_PROPERTY,
				GravatarProperties.RATING_DEFAULT);
	}

	/**
	 * Determine the default image parameter out of the configuration entry or
	 * the default value.
	 * 
	 * @return Gravatar default image parameter
	 */
	private String getDefaultImage() {
		String defaultImage = source.getPropertyValueElseDefault(
				GravatarProperties.DEFAULTIMAGE_PROPERTY,
	...
	/**
	 * Determine the needed image size out of the configuration entry or the
	 * default value.
	 * 
	 * @return The image size as value between 1 and 512
	 */
	private String getImageSize() {
		String size = source.getPropertyValueElseDefault(
				GravatarProperties.IMAGE_SIZE_PROPERTY,
	...
	public void dataSetChanged(Notification notification) {
		...
		if (notification.getEventType() == Notification.ADD
				...
				&& notification.getNewValue() instanceof Email) {
		...
		// information object got new Email
				...
				&& notification.getNotifier() instanceof InformationObject
				&& notification.getNewValue() instanceof Email) {
			// information object with new email
			InformationObject changedIO = (InformationObject) notification
					.getNotifier();
